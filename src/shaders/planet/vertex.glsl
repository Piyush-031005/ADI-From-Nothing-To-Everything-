...
// Planet vertex displacement shader
// Uses fbm noise to create terrain displacement
...
out float vHeat;   // 0=cool, 1=magma

// ── Noise helpers (inlined for vertex) ───────────────────────────────────────
...
void main() {
  ...
  float n = fbm(position * 2.0 + vec3(uTime * 0.02));
  float n2 = fbm(position * 4.0 + vec3(100.0));
  float elevation = n * 0.15 + n2 * 0.05;

  vElevation = elevation;
  vHeat = clamp(1.0 - (elevation + 0.2) / 0.4, 0.0, 1.0); // low = hot lava
