...
export class Era3_Stars {
  ...
  _buildStarClusters() {
    ...
    for (let i = 0; i < count; i++) {
      // Cluster distribution — gaussian blobs
      const clusterX = (Math.floor(Math.random() * 5) - 2) * 8;
      const clusterZ = (Math.floor(Math.random() * 5) - 2) * 8;
      positions[i*3]   = clusterX + (Math.random() - 0.5) * 12;
  ...
  hide(duration = 0.6) {
    this.visible = false;
    const start = performance.now();
    const startOp = this.stars.material.opacity;
    const tick = () => {
      const t = Math.min((performance.now() - start) / (duration * 1000), 1);
      this.stars.material.opacity = startOp * (1 - t);
